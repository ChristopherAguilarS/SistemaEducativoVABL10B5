...
class Inventariar extends Component
{
    ...
    #[On('delInv')]
    public function delInv($id =0, $nom =''){
        $this->idDel =$id;
        $this->dispatch('confirmar', ['mensaje' => 'Confirmación de Eliminacion', 'detalle' => 'Se eliminara el inventario #'.$nom, 'funcion' => 'brInv']);
    }
    #[On('brInv')]
    public function brInv(){
        $sav = EquipoInventario::destroy($this->idDel);
        $this->dispatch('rTabla2');
        $this->dispatch('alert_info', ['mensaje' => 'Eliminado Correctamente']);
    }
    public function render(){
        return view('livewire.patrimonio.por-ambiente.components.inventariar');
    }
}