...
class VerDetalles extends Component
{
    ...
    public function ver($id){
        if($id){
            $this->titulo = "Edición de Banco";
            $this->editar = true;
            $this->state = Banco::select('nombre', 'estado')->find($id)->toarray();
            $this->idSel = $id;
            $this->state['updated_by'] = auth()->user()->id;
            $this->state['updated_at'] = date('Y-m-d H:i');
        }else{
            $this->titulo = "Nuevo Banco";
            $this->editar = false;
    ...
    public function guardar() {
        ...
        if($this->editar){
            $existe = Banco::where('nombre', $this->state['nombre'])->whereNotIn('id', [$this->idSel])->first();
        }else{
            $existe = Banco::where('nombre', $this->state['nombre'])->first();
        }
        ...
        if($existe){
            ...
            try{
                DB::beginTransaction();
                    if($this->editar){
                        $sav = Banco::where('id', $this->idSel)->update($this->state);
                    }else{
                        $sav = Banco::create($this->state);
                    }
                DB::commit();
                $this->alert('success', 'Banco guardado correctamente.');
    ...
    public function render() {
        return view('livewire.rrhh.configuracion.bancos.ver-detalles');
    }
}
