...
use App\Models\RecursosHumanos\LegEstudioPreGrado;
...
use DB;
class PreGrado extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    #[On('preGrado')]
    public function preGrado($id = 0){
        $this->idSel = 0;
        $this->reemplaza = false;
        $this->archivo = null;
        $this->titulo = "Estudios de Pre Grado";
    ...
    public function eliminar($id){
        $this->idDel = $id;
        $this->dispatch('confirmar', ['mensaje' => 'Confirmación de Eliminacion', 'detalle' => 'Se eliminara el estudio #'.($id + 1), 'funcion' => 'brEst']);
    }
    ...
    public function buscar(){
        ...
        $data = LegEstudioPreGrado::select('institucion_nombre', 'catalogo_tipo_estudio_id', 'programa', 'estado_estudio', 'fecha_emision', 'fecha_inicio', 'fecha_fin', DB::raw("'1' as estado"))->where('persona_id', $this->idSel)->get();
        if($data){
            $this->estudios = $data->toarray();
        }else{
            $this->estudios = null;
        }
        $archivo = 'legajos/'.$this->idSel.'/pregrado.pdf';
    ...
    public function guardar(){
        try {
            $del = LegEstudioPreGrado::where('persona_id', $this->idSel)->delete();
            if($this->estudios){
                foreach ($this->estudios as $data) {
                    if($data['estado']){
                        ...
                        LegEstudioPreGrado::create($data);
                    }
                }
            }
            if ($this->archivo) {
                if($this->existe && !$this->reemplaza){
                    ...
                    $pdfPathB = public_path('legajos/'.$this->idSel.'/pregrado.pdf');
                    $pdfMerge->add($pdfPathB);
                    $pdfMerge->merge(public_path('legajos/'.$this->idSel.'/pregrado.pdf'));
                    Storage::disk('public')->delete($rutaTemporalA);
                }else{
                    $file = $this->archivo->getClientOriginalName();
                    $extension = pathinfo($file, PATHINFO_EXTENSION);
                    $nombre = 'pregrado.'.$extension;
    ...
    public function render(){
        return view('livewire.rrhh.personal.components.pre-grado');
    }
}
