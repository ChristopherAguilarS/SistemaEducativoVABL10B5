...
class VerMerito extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    #[On('nuevoMerito')]
    public function nuevoMerito($id = 0){
        if($id){
            $this->titulo = "Editar Nivel Academico";
            $this->state = Carrera::find($id)->toarray();
            $this->editar = $id;
        }else{
            $this->titulo = "Ingreso de Meritos del Trabajador";
            $this->regimen = 0;
            $this->idSel = 0;
            $this->documento = '';
            $this->nombres = '';
    ...
    public function buscar(){
        ...
        if($data){
            if($data->estado == 1){
                ...
                $this->idPers = $data->id;
                $archivo = 'legajos/'.$this->idPers.'/meritos.pdf';
    ...
    public function guardar(){
        ...
        try {
            if($this->editar){
                ...
                $nivel = Merito::create($this->state);
            }
            if ($this->archivo) {
                if($this->existe && !$this->reemplaza){
                    ...
                    $pdfPathB = public_path('legajos/'.$this->idPers.'/meritos.pdf');
                    $pdfMerge->add($pdfPathB);
                    $pdfMerge->merge(public_path('legajos/'.$this->idPers.'/meritos.pdf'));
                    Storage::disk('public')->delete($rutaTemporalA);
                }else{
                    $file = $this->archivo->getClientOriginalName();
                    $extension = pathinfo($file, PATHINFO_EXTENSION);
                    $nombre = 'meritos.'.$extension;
    ...
    public function render(){
        ...
        return view('livewire.rrhh.trabajadores.components.ver-merito');
    }
}
