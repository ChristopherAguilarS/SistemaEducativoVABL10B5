...
class VerDetalles extends Component
{
    ...
    public function onNuevo($id = 0){
        if($id){
            ...
            $this->state = Semestre::find($id)->toarray();
            $this->editar = $id;
        }else{
    ...
    public function guardar(){
        ...
        try {
            if($this->editar){
                $nivel = Semestre::find($this->editar);
                ...
                $nivel = Semestre::create($this->state);
            }
    ...
    public function render(){
        return view('livewire.academico.administracion.semestres.ver-detalles');
    }
}
