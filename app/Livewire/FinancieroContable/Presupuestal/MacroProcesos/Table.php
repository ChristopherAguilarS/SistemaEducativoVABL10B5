...
class Table extends Component
{
    ...
    {
        $macroProcesos = MacroProceso::when($this->tipoProcesoId != null, function ($query) {
            return $query->where('tipo_proceso_id',$this->tipoProcesoId);
        })
        ->when($this->busqueda != null, function ($query) {
                return $query->where('descripcion', 'like', '%' . $this->busqueda . '%');
            })->paginate(10);
        return view('livewire.financiero-contable.presupuestal.macro-procesos.table',['macroProcesos'=>$macroProcesos]);
    }
}
