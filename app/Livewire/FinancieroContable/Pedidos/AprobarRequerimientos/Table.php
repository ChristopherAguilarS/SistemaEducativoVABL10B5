...
use App\Livewire\Forms\AprobarDetalleRequerimientoForm;
...
class Table extends Component
{
    ...
    public AprobarDetalleRequerimientoForm $formDetalle;
    public $bienes;
    ...
    public function aprobarRequerimiento(){
        try {
            $requerimiento = Requerimiento::find($this->requerimientoId);
            $requerimiento->estado_proceso = 2;
            $requerimiento->comentarios = $this->form->comentarios;
            $requerimiento->save();
            foreach ($this->bienes as $key => $bien) {
                $requerimientoTemp = RequerimientoItem::where('item_id',$bien['item_id'])->where('requerimiento_id',$this->requerimientoId)->where('estado',1)->first();
                $requerimientoTemp->cantidad_aprobada = $bien['cantidad_aprobada'];
                $requerimientoTemp->observaciones = $bien['observaciones'];
                $requerimientoTemp->save();
            }
            $this->cerrarModal();            
            $this->mensajedeExito();
        } catch (\Exception $e) {
    ...
    public function abrirAprobacion($id){
        $this->requerimientoId = $id;
        $requerimiento = Requerimiento::find($id);
        ...
        $responsable = Persona::find($requerimiento->responsable_id);
        $this->responsable = optional($responsable)->nombres.' '.optional($responsable)->apellidoPaterno.' '.optional($responsable)->apellidoMaterno;
        foreach ($bienes as $key => $bien) {
            ...
            $this->bienes[$key]['cantidad_aprobada'] = $bien->cantidad_solicitada;
            $this->bienes[$key]['comentarios'] = "";
            $this->bienes[$key]['especificaciones'] = $bien->especificaciones;
        }
    }
    ...
    public function mensajedeExito()
    {
        if($this->requerimientoId == null){
            $this->mensaje = "Requerimiento aprobado exitosamente";
        }
        else{
            $this->mensaje = "Requerimiento aprobado exitosamente";
        }
    ...
    public function render()
    {
        $requerimientos = Requerimiento::when($this->fecha_inicio != null, function ($query) {
            return $query->where('fecha_registro_requerimiento','>=',$this->fecha_inicio);
        })
        ->when($this->fecha_fin != null, function ($query) {
            return $query->where('fecha_registro_requerimiento','<=',$this->fecha_fin);
        })->paginate(10);
        return view('livewire.financiero-contable.pedidos.aprobar-requerimientos.table',['requerimientos'=>$requerimientos]);
    }
}
