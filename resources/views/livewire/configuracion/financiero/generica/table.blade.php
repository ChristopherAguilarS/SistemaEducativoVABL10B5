                                ...
                                @foreach ($genericas as $generica)
                                    <tr>                
                                        ...
                                        <td class="text-center">
                                            @if($generica->estado == 1)
                                                <button type="button" class="btn btn-danger bg-gradient waves-effect waves-light" wire:click='cambiarEstado({{ $generica->id }})'>Dar de Baja <i class="ri-thumb-down-line align-bottom me-1"></i></button>
                                            
                                            @else
                                                <button type="button" class="btn btn-success bg-gradient waves-effect waves-light" wire:click='cambiarEstado({{ $generica->id }})'>Dar de Alta <i class="ri-thumb-up-line align-bottom me-1"></i></button>
                                            @endif
                                                <button type="button" class="btn btn-info bg-gradient waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#myModal" wire:click='editar({{ $generica->id }})'>Editar <i class="las la-edit"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        <div class="mb-3"  wire:ignore>
                            ...
                            <select class="js-example-basic-single" id="tipo_transaccion_id" name="state">
                                ...
                                @foreach ($tipo_transacciones as $tipo_transaccion)
                                    <option value="{{ $tipo_transaccion->id }}">{{ $tipo_transaccion->id }} - {{ $tipo_transaccion->descripcion }}</option>
                                @endforeach
                            </select>
                        </div>                        
                        <div>
                            @error('form.tipo_transaccion_id') <span class="error">{{ $message }}</span> @enderror
                        </div>
                    </form>
                </div>
