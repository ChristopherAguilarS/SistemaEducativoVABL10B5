                ...
                <div class="card-body">
                    <div class="table-responsive table-card">
                        <table class="table table-nowrap table-striped-columns mb-4">
                            ...
                            <tbody>
                                @foreach ($añoAcademicos as $añoAcademico)
                                    <tr>                
                                        ...
                                        <td>
                                            @if($añoAcademico->estado == 1)
                                                <span class="badge bg-success">{{ $añoAcademico->nEstado }}</span>
                                            @else
                                                <span class="badge bg-danger">{{ $añoAcademico->nEstado }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            @if($añoAcademico->estado == 1)
                                                <button type="button" class="btn btn-danger bg-gradient waves-effect waves-light" wire:click='cambiarEstado({{ $añoAcademico->id }})'>Dar de Baja <i class="ri-thumb-down-line align-bottom me-1"></i></button>
                                            
                                            @else
                                                <button type="button" class="btn btn-success bg-gradient waves-effect waves-light" wire:click='cambiarEstado({{ $añoAcademico->id }})'>Dar de Alta <i class="ri-thumb-up-line align-bottom me-1"></i></button>
                                            @endif
                                                <button type="button" class="btn btn-info bg-gradient waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#myModal" wire:click='editar({{ $añoAcademico->id }})'>Editar <i class="las la-edit"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
